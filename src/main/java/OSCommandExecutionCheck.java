...
@Rule(  key = "OSCommandExecutionCheckRule",
        priority = Priority.CRITICAL,
        name = "OSCommandExecutionCheck",
        description = "Check method and its parameter that executes OS command"  )
public class OSCommandExecutionCheck extends IssuableSubscriptionVisitor {
    ...
    private static final MethodMatcherCollection COMMAND_EXECUTION_SUSPECTS = MethodMatcherCollection.create(
             ...
             */
            matcherBuilder().name("exec").withAnyParameters()
    );

    private static MethodMatcher matcherBuilder() {
        return MethodMatcher.create().typeDefinition(TypeCriteria.subtypeOf(OSCommandExecutionCheck.JAVA_LANG_RUNTIME));
    }

    @Override
    public List<Kind> nodesToVisit() {
        // checks the symbol of invocated method,
        return Arrays.asList(Kind.METHOD_INVOCATION, Kind.BLOCK);
    }
    ...
    private static Stream<ExpressionTree> arguments(Tree methodTree) {
        if (methodTree.is(Kind.METHOD_INVOCATION)) {
    ...
    private static boolean isDynamicString(ExpressionTree arg) {
        ...
        return arg.is(Kind.PLUS) && !arg.asConstant().isPresent();
    }

    private static boolean checkIfStatement(Tree tree) {
        return Objects.requireNonNull(tree.parent()).is(Kind.IF_STATEMENT);
    }
}
